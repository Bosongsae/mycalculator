...
function input(number: string) {
  ...
  // 숫자 다음에 숫자를 입력하는 경우 지우는 요소 없이 number 추가 (기존숫자 자리수 증가)
    ...
    if (inputtext.textContent.length > 3) {
      numAlert()
    } else {
    inputtext.textContent += number;
    lastinputs += number;
...
function calculator(numbers : Array<number>, ops : Array<string>) : number[] {
    while (ops.indexOf("*") !== -1 || ops.indexOf("/") !== -1) {
      let xarry = ops.map((item, index) => item === "*" ? index : -1).filter(index => index !== -1);
      let sarry = ops.map((item, index) => item === "/" ? index : -1).filter(index => index !== -1);
      let combined = xarry.concat(sarry).sort();
      for (let i=0; i < combined.length; i++) {
        let a = numbers.splice(combined[i], 1);
        let b = numbers.splice(combined[i], 1);
        if (ops[combined[i]] === '*') {
          let newnumber : number = a[0] * b[0];
          numbers.splice(combined[i], 0, newnumber);
          ops.splice(combined[i], 1);
        } else { let newnumber : number = a[0] / b[0];
          numbers.splice(combined[i], 0, newnumber);
          ops.splice(combined[i], 1);
        }
      }
    ...
    while (ops.indexOf("+") !== -1 || ops.indexOf("-") !== -1) {
      ...
      let sarry = ops.map((item, index) => item === "-" ? index : -1).filter(index => index !== -1);
      let combined = xarry.concat(sarry).sort();
      console.log("+/- 계산시", numbers);
      console.log("+/- 계산시", ops);
      console.log("+/- 계산시", combined);
      for (let i=0; i < combined.length; i++) {
        let a = numbers.splice(combined[0], 1);
        let b = numbers.splice(combined[0], 1);
        console.log("더하기 전 a", a);
        console.log("더하기 전 b", b);
        if (ops[combined[i]] === '+') {
          let newnumber : number = a[0] + b[0];
          numbers.splice(combined[0], 0, newnumber);
          ops.splice(combined[0], 1);
          console.log("더한 후", numbers);
          console.log("더한 후", ops);
        } else { let newnumber : number = a[0] - b[0];
          numbers.splice(combined[i], 0, newnumber);
          ops.splice(combined[i], 1);
        }
